...
Name: "Airwave." The radio.garden-style interactive globe is now a pinned, durable product decision — it stays through future visual redesigns. Its skin, however, has iterated repeatedly: an initial "deep-space cosmic" pass (teal/magenta neon-glass), then an aizuri-e (Hokusai-style indigo woodblock print) globe with real landmass silhouettes, then a night-earth world (city-light glow on black continents) pinned via a Dribbble reference — both of which the user ultimately felt were the same "dark globe with dots" family. The current skin is a celestial star atlas, won by an Impeccable direction round's own catalog challenger against a dice-assigned storm/radar-chart direction: a navy chart sphere with no landmass or ocean at all — a faint RA/Dec coordinate graticule, scattered chalk-white backdrop stars with a few connecting constellation lines, and station pins rendered as the chart's real stars, sized by real listener clickcount (a true magnitude ramp). The tuned-in station wears a steady red night-vision ring rather than a pulsing glow. A dim red-flashlight rim traces the limb. Floating chrome (nav pill, player dock, now-playing card, auth form) stays the earlier aizuri-e system: opaque cream "paper card" stock with a hairline ink border, not frosted glass. Fraunces (serif, for names/headings) plus Inter (sans, for UI) carry the type. Held at full fidelity, not a placeholder.
